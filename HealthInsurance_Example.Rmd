...
corrdata <- reorder_cormat(corrdata)
...
### Melt
corrdata_tall <- melt(lower_tri, na.rm = TRUE)
...
## Multiple correlation plot (Smoker+Other Variables)
...
## Using the model matrix function. ~ to set comparison, 0+ to include intercept (omit if not needed), and "." to indicate that we wish to compare all variables.

model.matrix(~0+charges*age*bmi*smoker, data=data_fixed) %>% 
  cor(use="pairwise.complete.obs") %>% 
...
extract_eq <- function(lm_fit,digits){
        ...
        equation_result <- paste(equation_result,fitdata$operator[i],round(abs(fitdata$estimate[i]),digits),fitdata$variable[i]," ",sep="")
...
```{r predvsactual_test,message=FALSE,fig.width=10,fig.height=5}
...
rmse_train <- charge_test_results_train %>% 
  rmse(truth = charges, estimate = .pred) %>%
  mutate(dataset = "train") %>%
  dplyr::select(dataset,.estimate)

rmse_test <- charge_test_results_test %>% 
  rmse(truth = charges, estimate = .pred) %>%
  mutate(dataset = "test") %>%
  dplyr::select(dataset,.estimate)        

rmse_combined <- rbind(rmse_train,rmse_test) %>%
  mutate(dataset = fct_relevel(dataset,"train", "test"))

```

```{r summary stats}

summary_stats_train <- charge_test_results_train %>% 
  summarize(dataset = "train",average = mean(charges,na.rm=TRUE),median = median(charges,na.rm=TRUE),standdev = sd(charges,na.rm=TRUE)) 

summary_stats_test <- charge_test_results_test %>% 
  summarize(dataset = "test",average = mean(charges,na.rm=TRUE),median = median(charges,na.rm=TRUE),standdev = sd(charges,na.rm=TRUE)) 

summary_stats <- rbind(summary_stats_train,summary_stats_test)
...
rmse_combined <- rmse_combined %>%
        left_join(summary_stats,by="dataset") %>%
        dplyr::mutate(diff = .estimate / standdev) %>%
  mutate(dataset = fct_relevel(dataset,"train", "test"))

```


```{r rmse_plot,message=FALSE,fig.width=10,fig.height=5}

p <- ggplot(rmse_combined,(aes(dataset,diff,fill=dataset,color=dataset))) +
  geom_bar(stat="identity",position="dodge") +
  xlab("Dataset") + 
  ylab(expression("RMSE Value / Standard Deviation")) +
  ggtitle("Multiple Regression Model has less variance for testing set than training set") +
  theme_light() +
  scale_color_colorblind() +
  scale_fill_colorblind() +
  theme(axis.text.x=element_text(size=rel(1.2),angle = 90, vjust = 0.5, hjust = 1),
...
### Construct a simple regression model using Smoker status to predict cost
...
### Compare Smoking to Multivariate Model for Healthcare Costs using R-Squared Method

```{r r-squared_plot,message=FALSE,fig.width=10,fig.height=5}
...
rsq_combined <- rbind(rsq_train,rsq_test,rsq_smoker) %>%
  mutate(dataset = fct_relevel(dataset,"Train", "Test","Test (Smoker only)"))

p <- ggplot(rsq_combined,(aes(dataset,.estimate,fill=dataset,color=dataset))) +
  geom_bar(stat="identity",position="dodge") +
  xlab("Dataset") + 
  ylab(expression("R-Squared Value")) +
  ggtitle("Multiple Regression Model explains more data variance than training dataset and smoking model") +
  theme_light() +
  scale_color_colorblind() +
  scale_fill_colorblind() +
  theme(axis.text.x=element_text(size=rel(1.2),angle = 90, vjust = 0.5, hjust = 1),
